...
public class CommonSteps {
    ...
    public void the_response_contains_system_data(DataTable data) throws UnsupportedEncodingException, JsonProcessingException {
    ...
    public void the_database_only_contains_records(DataTable dataTable) {
        ...
        for (Map<String, String> expectedSystem : dataTable.asMaps()) {
            Optional<SystemEntity> optionalSystemEntity =
                    systemEntities.stream()
                            .filter(hasSystemFields(expectedSystem))
                            .findFirst();
    ...
    private Predicate<SystemEntity> hasSystemFields(
            Map<String, String> expectedSystem) {
        return systemEntity ->
                compareIfPresent(
                        expectedSystem.get("id"), systemEntity.getId().toString())
                        && compareIfPresent(
                        expectedSystem.get("code"),
                        systemEntity.getCode())
                        && compareIfPresent(
                        expectedSystem.get("name"), systemEntity.getName())
    ...
    private SystemEntity createSystemEntity(Map<String, String> systemFields) {
        return databaseDelegate.createSystem(
                systemFields.get("code"),
                systemFields.get("name"),
                systemFields.get("description"));
    }
}
