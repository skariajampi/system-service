...
class AdaasSystemControllerTest {
    ...
    void getAdaasSystem_givenRecordExists_thenReturnData() throws Exception {
        ...
        // Given
        SystemResponse response = SystemResponse.builder().code("code").description("description").name("name").build();

        given(adaasSystemService.getAdaasSystem(any()))
                .willReturn(response);

        // When
        mockMvc.perform(
                        get("/systems/{code}", "AFN"))
    ...
    void getAdaasSystems_givenRecordsExist_thenReturnList() throws Exception {
        ...
        given(adaasSystemService.getAdaasSystems())
                .willReturn(adaasSystemList);
        ...
        assertThat(systems).hasSameSizeAs(adaasSystemList)
                .extracting("code").contains("code1", "code2");
    ...
    @Test
    @WithAwsCognitoUser(authority = "MyGreeterLambda")
    void updateAdaasSystem_givenRecordDoesNotExist_thenUpdateFailsAndReturnsNotFoundException() throws Exception {
